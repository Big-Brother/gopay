...
export class ProfileProvider {
  ...
  private bindWalletClient(wallet: any, opts?: any): boolean {
    ...
    wallet.gopayerId = wallet.credentials.gopayerId;
  ...
  public importWallet(str: string, opts: any): Promise<any> {
    return new Promise((resolve, reject) => {
      ...
      if (!strParsed.n) {
        return reject("Backup format not recognized. If you are using a Gopay Beta backup and version is older than 0.10, please see: https://github.com/squarepay/gopay/issues/4730#issuecomment-244522614");
      }
  ...
  public isDisclaimerAccepted(): Promise<any> {
    return new Promise((resolve, reject) => {
      ...
      // OLD flag
      this.persistenceProvider.getGopayDisclaimerFlag().then((val) => {
        if (val) {
  ...
  public isOnboardingCompleted(): Promise<any> {
    return new Promise((resolve, reject) => {
      ...
      this.persistenceProvider.getGopayOnboardingFlag().then((val) => {
        if (val) {
    ...
    return new Promise((resolve, reject) => {
      ...
      let pr = (wallet, cb) => {
        updateNotifications(wallet).then(() => {
          ...
          let idToName = {};
          if (wallet.cachedStatus) {
            _.each(wallet.cachedStatus.wallet.gopayers, (c: any) => {
