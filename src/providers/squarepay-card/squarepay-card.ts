...
@Injectable()
export class SquarePayCardProvider {
  ...
  constructor(
    ...
    this.logger.info('SquarePayCardProvider initialized');
  }
  ...
  public sync(apiContext, cb) {
    ...
    this.bitPayProvider.post('/api/v2/' + apiContext.token, json, (data) => {
      ...
      this.logger.info('SquarePay Get Debit Cards: SUCCESS');
      ...
      _.each(data.data, (x) => {
        ...
        if (!x.eid || !x.id || !x.lastFourDigits || !x.token) {
          this.logger.warn('BAD data from SquarePay card' + JSON.stringify(x));
          return;
        }
      ...
      this.persistenceProvider.setSquarepayDebitCards(this.bitPayProvider.getEnvironment().network, apiContext.pairData.email, cards).then(() => {
        return cb(null, cards);
      });
    }, (data) => {
      return cb(this._setError('SquarePay Card Error: Get Debit Cards', data));
    });
  };
    ...
    this.appIdentityProvider.getIdentity(this.bitPayProvider.getEnvironment().network, (err, appIdentity) => {
      ...
      this.getCards((data) => {
        ...
        this.bitPayProvider.post('/api/v2/' + card.token, json, (data) => {
          this.logger.info('SquarePay Get Invoices: SUCCESS');
          invoices = data.data || [];
          ...
          this.bitPayProvider.post('/api/v2/' + card.token, json, (data) => {
            this.logger.info('SquarePay Get History: SUCCESS');
            history = data.data || {};
            ...
            return cb(this._setError('SquarePay Card Error: Get History', data));
          });
        }, (data) => {
          return cb(this._setError('SquarePay Card Error: Get Invoices', data));
        });
      });
    });
  };
    ...
    this.appIdentityProvider.getIdentity(this.bitPayProvider.getEnvironment().network, (err, appIdentity) => {
      ...
      this.getCards((data) => {
        ...
        this.bitPayProvider.post('/api/v2/' + card.token, json, (res) => {
          this.logger.info('SquarePay TopUp: SUCCESS');
          if (res.error) {
            return cb(res.error);
          } else {
          ...
          return cb(this._setError('SquarePay Card Error: TopUp', res));
        });
      });
    });
  };
  ...
  public getInvoice(id, cb) {
    this.bitPayProvider.get('/invoices/' + id, (res) => {
      this.logger.info('SquarePay Get Invoice: SUCCESS');
      return cb(res.error, res.data);
    }, (res) => {
      return cb(this._setError('SquarePay Card Error: Get Invoice', res));
    });
  };

  // get all cards, for all accounts.
  public getCards(cb) {
    this.persistenceProvider.getSquarepayDebitCards(this.bitPayProvider.getEnvironment().network).then((val) => {
      return cb(val);
    });
  };
  ...
  public remove(cardId, cb) {
    this.persistenceProvider.removeSquarepayDebitCard(this.bitPayProvider.getEnvironment().network, cardId).then(() => {
      this.persistenceProvider.removeBalanceCache(cardId);
      return cb();
    }).catch((err) => {
      this.logger.error('Error removing SquarePay debit card: ' + err);
      return cb(err);
    });
  };
  ...
  public getRates(currency, cb) {
    this.bitPayProvider.get('/rates/' + currency, (data) => {
      this.logger.info('SquarePay Get Rates: SUCCESS');
      return cb(data.error, data.data);
    }, (data) => {
      return cb(this._setError('SquarePay Error: Get Rates', data));
    });
  };
  ...
  public register() {
    this.isActive((isActive) => {
      this.homeIntegrationsProvider.register({
        name: 'debitcard',
        title: 'SquarePay Visa® Card',
        icon: 'assets/img/squarepay-card/icon-squarepay.svg',
        page: 'SquarePayCardIntroPage',
