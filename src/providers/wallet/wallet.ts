...
export class WalletProvider {
  ...
  // https://github.com/squarepay/bitcore-wallet-service/blob/master/lib/model/txproposal.js#L243
  ...
  public copyGopayers(wallet: any, newWallet: any): Promise<any> {
    return new Promise((resolve, reject) => {
      ...
      lodash.each(wallet.credentials.publicKeyRing, (item) => {
        let name = item.gopayerName || ('gopayer ' + gopayer++);
        newWallet._doJoinWallet(newWallet.credentials.walletId, walletPrivKey, item.xPubKey, item.requestPubKey, name, {
          ...
          // Ignore error is gopayer already in wallet
