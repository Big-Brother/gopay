...
export class IncomingDataProvider {
  ...
  public redir(data: string): boolean {
    ...
    if (this.bwcProvider.getBitcore().URI.isValid(data)) {
      ...
      switch (reason) {
        ...
        case '0':
          /* For SquarePay card binding */
          this.navCtrl.push(SquarePayCardIntroPage, { secret, email, otp });
          break;
      }
      return true;

      // Join
    } else if (data && data.match(/^gopay:[0-9A-HJ-NP-Za-km-z]{70,80}$/)) {
  ...
  private sanitizeUri(data: any): string {
    ...
    // mobile devices, uris like gopay://glidera
    newUri.replace('://', ':');
