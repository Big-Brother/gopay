...
@Component({
  selector: 'page-squarepay-card-topup',
  templateUrl: 'squarepay-card-topup.html',
})
export class SquarePayCardTopUpPage {
  ...
  ionViewDidLoad() {
    this.logger.info('ionViewDidLoad SquarePayCardTopUpPage');
  }
  ...
  private openFinishModal(): void {
    ...
    modal.onDidDismiss(() => {
      this.navCtrl.popToRoot({ animate: false }).then(() => {
        ...
        setTimeout(() => {
          this.navCtrl.push(SquarePayCardPage, { id: this.cardId }, { animate: false });
        }, 200);
      });
    });
  }
