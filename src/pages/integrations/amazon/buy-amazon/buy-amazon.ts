...
export class BuyAmazonPage {
  ...
  private createInvoice(data: any): Promise<any> {
    return new Promise((resolve, reject) => {
      this.amazonProvider.createSquarePayInvoice(data, (err: any, dataInvoice: any) => {
        if (err) {
        ...
        this.amazonProvider.getSquarePayInvoice(dataInvoice.invoiceId, (err: any, invoice: any) => {
          if (err) {
            return reject({
