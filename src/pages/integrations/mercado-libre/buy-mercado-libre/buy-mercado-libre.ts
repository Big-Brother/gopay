...
export class BuyMercadoLibrePage {
  ...
  private createInvoice(data: any): Promise<any> {
    return new Promise((resolve, reject) => {
      this.mercadoLibreProvider.createSquarePayInvoice(data, (err: any, dataInvoice: any) => {
        if (err) {
        ...
        this.mercadoLibreProvider.getSquarePayInvoice(dataInvoice.invoiceId, (err: any, invoice: any) => {
          if (err) {
            return reject({
