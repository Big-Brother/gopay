...
@Component({
  selector: 'page-gopayers',
  templateUrl: 'gopayers.html',
})
export class GopayersPage {
  ...
  ionViewWillEnter() {
    this.logger.info('ionViewDidLoad GopayersPage');
    this.wallet = this.profileProvider.getWallet(this.navParams.data.walletId);
    this.updateWallet();

    this.events.subscribe('bwsEvent', (walletId, type, n) => {
      if (this.wallet && walletId == this.wallet.id && type == ('NewGopayer' || 'WalletComplete')) {
        this.updateWallet();
      }
    });
  }
