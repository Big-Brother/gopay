...
export class BitcoinCashPage {
  ...
  public openRecoveryToolLink(): void {
    let url = 'https://squarepay.github.io/gopay-recovery/';
    let optIn = true;
    let title = null;
  ...
  public duplicate(wallet: any) {
    ...
    let opts: any = {
      ...
      m: wallet.m,
      n: wallet.n,
      myName: wallet.credentials.gopayerName,
    ...
    // Multisig wallets? add Gopayers
    function addGopayers(newWallet, isNew, cb) {
      if (!isNew) return cb();
      if (wallet.n == 1) return cb();

      this.logger.info('Adding gopayers for BCH wallet config:' + wallet.m + '-' + wallet.n);

      this.walletProvider.copyGopayers(wallet, newWallet, (err) => {
        if (err) {
          return cb(err);
        }
        return cb();
    ...
    this.walletProvider.getKeys(wallet).then((keys) => {
      ...
      importOrCreate().then((result: any) => {
        ...
        addGopayers(newWallet, isNew, (err) => {
