...
export class WalletTransactionHistoryPage {
  ...
  public csvHistory() {
    ...
    this.walletProvider.getTxHistory(this.wallet, {}).then((txs: any) => {
      ...
      this.csvHeader = ['Date', 'Destination', 'Description', 'Amount', 'Currency', 'Txid', 'Creator', 'Gopayers', 'Comment'];

      var _amount, _note, _gopayers, _creator, _comment;
      ...
      data.forEach((it, index) => {
        ...
        if (it.actions && it.actions.length > 1) {
          for (var i = 0; i < it.actions.length; i++) {
            _gopayers += it.actions[i].gopayerName + ':' + it.actions[i].type + ' - ';
          }
