...
import { GopayersPage } from '../add/gopayers/gopayers';
import { AmazonPage } from '../integrations/amazon/amazon';
import { SquarePayCardPage } from '../integrations/squarepay-card/squarepay-card';
import { SquarePayCardIntroPage } from '../integrations/squarepay-card/squarepay-card-intro/squarepay-card-intro';
...
export class HomePage {
  ...
  ionViewDidEnter() {
    ...
    this.bitPayCardProvider.get({}, (err, cards) => {
      this.zone.run(() => {
        this.showSquarePayCard = this.appProvider.info._enabledExtensions.debitcard
          ? true
          : false;
  ...
  private listenForEvents() {
    ...
    // NewBlock, NewGopayer, NewAddress, NewTxProposal, TxProposalAcceptedBy, TxProposalRejectedBy, txProposalFinallyRejected,
  ...
  private handleDeepLinksNW() {
    ...
    // This event is sent to an existent instance of Gopay (only for standalone apps)
    gui.App.on('open', this.onOpenNW.bind(this));

    // Used at the startup of Gopay
  ...
  public goToWalletDetails(wallet: any): void {
    ...
    if (!wallet.isComplete()) {
      this.navCtrl.push(GopayersPage, {
  ...
  public goToDownload(): void {
    let url = 'https://github.com/squarepay/gopay/releases/latest';
    let optIn = true;
  ...
  public goToCard(cardId): void {
    this.navCtrl.push(SquarePayCardPage, { id: cardId });
  }
