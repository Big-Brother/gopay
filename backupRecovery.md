...
# Gopay Backup and Restore Notes

## Description

Gopay is a Multisig HD Wallet. Gopay app holds the extended private keys for the wallet. The private key never leaves the device, so for accessing a wallet funds it is necesary to have the device or a backup of the wallet.

## Definitions

### Backup Formats:
 * Wallet recovery phrase (RP): 12 words mnemonic backup (available from Gopay v1.2+). The 12 words are used as wallet seed, following BIP39 specification. The wallet RP may require a passphrase to recreate the wallet (if one was specified at creation time).
 * Wallet Backup (WB): Exported data from Gopay, containing an AES encrypted JSON with many wallet parameters (like extended private key, wallet name, extended public keys of gopayers, etc. See #export-format). This data can be created from Gopay v1.2+ (Settings -> Advanced -> Export) and it was the default backup format on previous Gopay versions. WB can be a file (standard format for Gopay desktop versions) or a text (standard for Gopay mobile versions).
 ...
 * Full Recovery: All the features of Partial Recovery + wallet name, gopayer names are recovered, past payment proposal metadata (who signed, and notes) are recovered.
...
### Non-multisig wallets
  ...
  Case 3: From both Backup Words and Backup file, basic recovery is possible.
    ...
    - Enter the RP at 'Import Wallet'
      ...
      - Go to 'Create Wallet', and enter the RP at 'Advanced Options'. Select a new name for the restored wallet. Total and required number of gopayers should be set to 1.
      - Wallet should be recreated and access to funds should be restored.
...
### Multisig wallets
  ...
  Case 3: Basic recovery is possible using:
    ...
    A) RP of all gopayers in the wallet
      - Enter one RP at Create (at the Advanced option section). Note that the wallet configuration (M-of-N and network paramenters) needs to match the parameters that where entered when the wallet was first created. Wallet name and gopayer nicknames need to be entered also, but there is no need for them to match the original wallet setup.
      - Ask other gopayers to join the wallet using the given invitation code. All gopayers need to enter their RP at Join (at -> Advanced Options -> Wallet Seed).
      - Wallet should be recreated and access to funds should be restored.

    B) One WB and a quorum of RP of the other members.
      - Using the WB, import the wallet.
      - Ask other gopayers to import the wallet using their RP.
      - Wallet should be recreated and funds should be accesable

      In this case, Gopayers will not be able to decrypt the 'notes' field on the new Spend Proposals, because the shared secret stored at the WB is not longer known by other gopayers.